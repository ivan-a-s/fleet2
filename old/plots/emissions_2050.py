...
files = {
    ...
    'Foresight': '../Results_19_3_2026/Foresight.pkl',
    'Break Mandate': '../Results_19_3_2026/Break Mandate.pkl',
    'Accelerated Retirement': '../Results_19_3_2026/Accelerated Retirement.pkl',
    'ZEV Mandate': '../Results_19_3_2026/ZEV Mandate.pkl',
    'ZEV Mandate (Strong)': '../Results_19_3_2026/ZEV Mandate (Strong).pkl',
...
fig, ax = plt.subplots(1, 3, sharey=False, figsize=(10, 4), dpi=300, tight_layout=True)   # share y + less squish
ax = ax.flatten()
...
for label, fname in files.items():
    ...
    box_plot(combustion[:, -1], ax[0], x, annotate=annotate)
    ...
    labels.append(label)
    x += 1
    print(label, np.median(combustion[:, -1]), np.median(supply[:, -1]), np.median(total_fuel[:, -1]))
    percent = total_fuel[:, -1]/(base_total_fuel[:, 0]/1.3448) * 100
    print(np.percentile(percent, q=[5, 25, 50, 75, 95]))
    percent = 100 - total_fuel[:, -1]/(base_total_fuel[:, -1]) * 100
    print('base 2050 reduce', np.percentile(percent, q=[5, 25, 50, 75, 95]))
...
ax[0].set_ylabel(r'Emissions (MtCO$_2$e)')
