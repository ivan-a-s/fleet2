""" Generate plots for embodied emissions. """
import numpy as np
...
fig, ax = plt.subplots(1, 3, sharey=False, figsize=(10, 4), dpi=300)   # share y + less squish
ax = ax.flatten()
...
for label, fname in files.items():
    ...
    embodied = inner['Embodied']
    total = total_fuel + embodied

    box_plot(total_fuel[:, -1], ax[0], x)
    box_plot(embodied[:, -1], ax[1], x)
    box_plot(total[:, -1], ax[2], x)
...
labels = ['Fuel', 'Embodied', 'Total']
initial_emissions=[np.mean(base_total_fuel[:, 0]), np.mean(base_embodied[:, 0]), np.mean(base_total[:, 0])]
...
for label, fname in files.items():
    ...
    box_plot(embodied[:, 5], ax[0], x)
    box_plot(embodied[:, 15], ax[1], x)
    box_plot(embodied[:, -1], ax[2], x)

    print(label, 100*np.percentile(embodied[:, -1]/base_embodied[:, -1], [5, 95]))
    print(label, 100*np.percentile(embodied[:, 5]/base_embodied[:, 5], [5, 95]))
    print(label, 100*np.percentile(embodied[:, 15]/base_embodied[:, 15], [5, 95]))
...
for i, a in enumerate(ax):
    ...
    a.set_ylim(0, a.get_ylim()[1])
    axp = a.twinx()
    axp.set_ylim(0, a.get_ylim()[1]/(np.mean(base_embodied[:, 0])/1.3448) * 100)  # right y-axis as percentage
    a.set_ylabel("Emissions (MtCO2)")
    axp.set_ylabel(r"% of 2007 emissions")
...
np.percentile(total_fuel[:, -1]/np.mean(base_embodied[:, 0]), np.arange(100))
